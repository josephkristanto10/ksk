            ...
            <div class="modal-body">
                <form id="myforms">
                    <div class="form-group">
                        ...
                        <br>
                        <label for="cars">Department:</label>
                        <select id="departmentedit" name="departmentedit" class="form-control">
                            <?php
                                        for($i = 0 ; $i < count($mydepartment); $i++)
                                        {                                            
                                                echo '<option value="'.$mydepartment[$i]['id'].'">'.$mydepartment[$i]['department'].'</option>';                                       
...
<script>
    ...
    function adddata() {
        ...
        if (arridselectedassetadd.length > 0) {
            var myselects = arridselectedassetadd;
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $("#fromroomedit").append(result).promise().done(function () {
                        if(!idglobalidfromroomsedit =="")
                        {
                            $('#fromroomedit').find('option[value="' + idglobalidfromroomsedit +
                                '"]').prop('selected', true);
                        }
                      
                    });
                    $("#toroomedit").append(result).promise().done(function () {
                        if(!idglobalidtoroomsedit =="")
                        {
                            $('#toroomedit').find('option[value="' + idglobalidtoroomsedit +
                                '"]').prop('selected', true);
                        }
...
$('#selectassetedit').on('shown.bs.modal', function () {
    ...
    function openmodaledits(element) {
        ...
        //optional
        var branch = $("#branch_" + myid).val();
        var department = $("#department_" + myid).val();
        var fromroom = $("#fromroom_" + myid).val();
        var toroom = $("#toroom_" + myid).val();
        var remarks = $("#remark"+myid).text();

        idglobalidtoroomsedit = toroom;
        idglobalidfromroomsedit = fromroom;
    
        $("#remarkedit").val(remarks);

        $('#branchedit option[value=' + branch + ']').prop('selected', true);
    ...
    function asseteditshow() {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                tipe: "getassettransactiondepartment",
