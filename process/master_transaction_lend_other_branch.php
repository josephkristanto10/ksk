...
if($tipe == "load")
{
    ...
    if(empty($search)) {
        ...
        OR lroom.room LIKE '%$search%' 
        OR tltd.approval LIKE '%$search%'
        OR tltd.status LIKE '%$search%' )  group by tltd.id ");
    }
...
else if($tipe == "getroom")
{
    ...
    $mydate = date("Y-m-d");
    $sql = "INSERT INTO `transaction_lend_to_department`
    VALUES (NULL, 'TRX-020', '$mybranch', '$mydepartment', '$myroom', '$convertstart', '$convertend', 'pending', 'waiting', '$mydate');";
    $res = $conn->query($sql);
    ...
    for($i = 0 ;  $i < count($myselectedlist) ; $i++)
    {
        ...
        $sqls = "INSERT INTO `transaction_lend_to_department_log` VALUES (NULL, '$last_id', '".$myselectedlist[$i]."')";
        $ress = $conn->query($sqls);
...
else if($tipe == "getdetailtransaction")
{
    ...
    $sql = "select tltdl.*, asset.name as assetname, asset.noasset  from transaction_lend_to_department_log tltdl inner join asset on asset.id = tltdl.idasset where idtransaksi = '$idtransaction'";
    $res = $conn->query($sql);
    $mystring = "";
    $mycountasset = 0;
    $mynumber = 1;
